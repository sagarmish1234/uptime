      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>
              Configure the target website you want to monitor. You'll
              find the advanced configuration below, in the advanced
              settings section.
            </SheetDescription>
          </SheetHeader>
          <Formik
            initialValues={{
              url: 'https://',
              checkFrequency: 'EVERY_3_MINUTES',
            }}
            onSubmit={async (values) => {
              console.log(values);
              const id = toast.loading('In progress');
              try {
                await client.post('/monitor/create', values);
